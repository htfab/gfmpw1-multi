...
# Reset input delay
set_input_delay [expr $::env(CLOCK_PERIOD) * 0.5 + 2] -clock [get_clocks {clk}] [get_ports {rst_n}]
...
if { $::env(IO_SYNC) } {
	...
	set_input_delay -max [expr $in_ext_delay + 4.55] -clock [get_clocks {clk}] [get_ports {in[*]}]
	set_input_delay -min [expr $in_ext_delay + 1.26] -clock [get_clocks {clk}] [get_ports {in[*]}]
}

# Input Transition
set_input_transition -max 0.38  [get_ports {in[*]}]
set_input_transition -min 0.05  [get_ports {in[*]}]
